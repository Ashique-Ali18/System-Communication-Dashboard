...
async function loadAll() {
  try {
    const [emails, sms, wa, stats] = await Promise.all([
      fetchJSON("api/email.php"),
      fetchJSON("api/sms.php"),
      fetchJSON("api/whatsapp.php"),
      fetchJSON("api/stats.php"),
    ]);
...
document.getElementById("formEmail")?.addEventListener("submit", async (e) => {
  ...
  try {
    await postJSON("api/email.php", { email_to: fd.get("email_to") });
    e.target.reset();
...
document.getElementById("formWa")?.addEventListener("submit", async (e) => {
  ...
  try {
    await postJSON("api/whatsapp.php", {
      mobile_number: fd.get("mobile_number"),
      message: fd.get("message"),
    });
    e.target.reset();
...
// Click any delete button in tables (event delegation)
...
document.getElementById("btnConfirmDelete")?.addEventListener("click", async () => {
  ...
  try {
    await postJSON("api/delete.php", { type: pendingDelete.type, id: pendingDelete.id });
...
/* =========================
   FUTURE (Optional): Export CSV
   - Add buttons in HTML with IDs:
     exportEmail, exportSms, exportWa
========================= */
...
document.getElementById("exportEmail")?.addEventListener("click", () => {
  // keys must match API response keys
